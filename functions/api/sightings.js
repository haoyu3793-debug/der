...
export async function onRequestPost({ request, env }) {
  ...
  // The handle is a nickname the browser generated, not an account. Keep it
  // short and strip anything that is not a plain character, so nobody can post
  // as "<script>" or as a 4000-character name that breaks the layout.
  const author = String(body.author || "@guest")
  ...
  return json({ id: id, delete_key: deleteKey, seen_at: seenAt.toISOString() }, 201);
}
